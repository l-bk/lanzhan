...
<link type="text/css" rel="Stylesheet" href="/static/kindeditor/themes/default/default.css" />
<!--  <script type="text/javascript" charset="utf-8" src="/static/kindeditor/kindeditor.js"></script> -->
  <script type="text/javascript" charset="utf-8" src="/static/kindeditor/kindeditor-all.js"></script>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$(document).on("keyup", "[data-act='innum']", function() {
			var v = $(this).val();
	...
	<ul class="nav nav-tabs">
		<li class="active"><a
			href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章<shiro:hasPermission
					name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission> <shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/save?listType=${listType }" method="post" class="form-horizontal">
		<form:hidden path="id" />
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:forEach var="c" items="${categorylist}" varStatus="i">
					<a class="att" onclick="choseattention('${c.category.id}','${c.category.name}');">${c.category.name}</a>
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name"
					labelValue="${article.category.name}" title="栏目" url="/cms/category/treeData" module="article" selectScopeModule="true"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="title" htmlEscape="false" maxlength="200" class="input-xxlarge required" style="text-align: left;" data-act="innum"
					data-target="#titleNum" data-length="25"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="shorttile" htmlEscape="false" maxlength="200" class="input-xlarge" data-act="innum" data-target="#shorttileNum"
					data-length="25" />
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="keywords" htmlEscape="false" maxlength="200" class="input-xlarge" data-act="innum" data-target="#keywordsNum"
					data-length="25" />
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:formtreeselect id="msgid" name="msgsource.sid" value="${article.msgid}" labelName="msgsource.name"
					labelValue="${article.msgsource.name}" title="来源" url="/cms/category/formTreeData" module="article" notAllowSelectRoot="false"
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:forEach var="t" items="${attlist}">
					<label> <form:checkbox path="posid" value="${t.position.posid}" />${t.position.description}
					</label>
				</c:forEach>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="hidden" id="image" name="image" value="${article.imageSrc}" /> <img src="${article.imageSrc}" id="smallimage"
					<c:if test="${empty article.imageSrc}"> stype="display:none"</c:if> width="50px" height="40px"> <input type="button"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea path="description" htmlEscape="false" rows="4" maxlength="200" class="input-xxlarge" style="width:800px" />
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<form:textarea id="content" htmlEscape="true" path="articleData.content" rows="4" maxlength="200" stlye="width:95%" />
			...
			<div class="controls">
				...
				<script type="text/javascript">
					...
					function articleSelectRefresh() {
						...
						for (var i = 0; i < articleSelect.length; i++) {
							$("#articleSelectList").append(
									"<li>" + articleSelect[i][1] + "&nbsp;&nbsp;<a href=\"javascript:\" onclick=\"articleSelectAddOrDel('"
					...
					$("#relationButton").click(
							function() {
								top.$.jBox.open("iframe:${ctx}/cms/article/selectList?pageSize=8", "添加相关", $(top.document).width() - 220,
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<div class="cls-exp">
					...
					<div class="cls-exp">
						<fmt:parseDate value="${article.timing}" pattern="yyyy-MM-dd HH:mm:ss" var="t" />
						<input id="timing" type="text" name="timing" maxlength="20" class="input-medium Wdate"
		...
		<shiro:hasPermission name="cms:category:edit">
			<div class="control-group" <c:if test="${!empty article.category.id}">style="display: none"</c:if>>
				<label class="control-label">自定义内容视图:</label>
				<div class="controls">
					<form:select path="customContentView" class="input-medium">
						<form:option value="${article.category.customContentView }" label="默认视图" />
		...
		<div class="form-actions" style="width: 100%; bottom: 30px">
			<shiro:hasPermission name="cms:article:edit">
				<input id="btnSubmit" class="btn btn-primary" type="submit" value="发 布" />&nbsp;</shiro:hasPermission>
