...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cms/articleTemp/?category.id=${articleTemp.category.id}">文章列表</a></li>
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/articleTemp/form?id=${articleTemp.id}&category.id=${articleTemp.category.id}'><c:param name='category.name' value='${articleTemp.category.name}'/></c:url>">文章<shiro:hasPermission name="cms:article:edit">${not empty articleTemp.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="articleTemp" action="${ctx}/cms/articleTemp/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
		<div class="control-group">
			<label class="control-label">归属栏目:</label>
			<div class="controls">
                <sys:treeselect id="category" name="category.id" value="${articleTemp.category.id}" labelName="category.name" labelValue="${articleTemp.category.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="title" htmlEscape="false" maxlength="200" class="input-xxlarge measure-input required" style="text-align: left;"/>
				<!-- &nbsp;<label>颜色:</label>
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:formtreeselect id="msgid" name="articleTemp.msgsource.sid"
					value="${articleTemp.msgid}" labelName="articleTemp.msgsource.name"
					labelValue="${articleTemp.msgsource.name}" title="栏目"
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<form:checkbox path="posid" value="${t.value}"/>${t.key}
				</c:forEach>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<input id="weightDate" name="weightDate" type="text" readonly="readonly" maxlength="20" class="input-small Wdate"
					value="<fmt:formatDate value="${articleTemp.weightDate}" pattern="yyyy-MM-dd"/>"
		...
		<div class="control-group">
			...
			<div class="controls">
                <input type="hidden" id="image" name="image" value="${articleTemp.imageSrc}" />
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<ol id="articleTempSelectList"></ol>
				...
				<script type="text/javascript">
					...
					function articleSelectRefresh(){
						...
						$("#articleTempSelectList").children().remove();
						for (var i=0; i<articleSelect.length; i++){
							$("#articleTempSelectList").append("<li>"+articleSelect[i][1]+"&nbsp;&nbsp;<a href=\"javascript:\" onclick=\"articleSelectAddOrDel('"+articleSelect[i][0]+"','"+articleSelect[i][1]+"');\">×</a></li>");
		...
		<div class="control-group">
			...
			<div class="controls">
				<input id="createDate" name="createDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${articleTemp.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<c:if test="${not empty articleTemp.id}">
			<div class="control-group">
				<label class="control-label">查看评论:</label>
				<div class="controls">
					<input id="btnComment" class="btn" type="button" value="查看评论" onclick="viewComment('${ctx}/cms/comment/?module=article&contentId=${articleTemp.id}&status=0')"/>
					<script type="text/javascript">
